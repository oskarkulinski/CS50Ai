...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        self.mark_safe(cell)
        self.knowledge.append(Sentence(self.get_neighbors(cell) - self.get_neighbors(cell).intersection(self.safes), count))
        ks = set()
        km = set()
        toAdd = list()
        for i in range(len(self.knowledge)):
            ...
            for j in range(i + 1, len(self.knowledge)):
                ...
                if self.knowledge[i].cells.issubset(self.knowledge[j].cells):
                    toAdd.append(Sentence(self.knowledge[j].cells - self.knowledge[i].cells, self.knowledge[j].count - self.knowledge[i].count))
    ...
    def make_random_move(self):
        ...
        if len(possibilities) > 0:
            return random.choices(list(possibilities))[0]
        else:
            return None

    def get_neighbors(self, cell):
        """
        Returns neighbors of a cell, within the board
        """
        neighbors = set()
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
